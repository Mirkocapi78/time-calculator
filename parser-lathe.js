// ────────────────────────────────────────────────────────────────
//  Time Calculator – parser‐lathe.js  (versione modale completa)
// ────────────────────────────────────────────────────────────────

/* 1) PARSER → dal testo ISO a un array di comandi normalizzati */
function parseISO(text) {
  const cmds = [];
  let state = {
    code: 'G0',      // movimento modale
    feedMode: 'G95', // mm/giro di default
  };

  for (const raw of text.split(/\r?\n/)) {
    // rimuove commenti e numeri linea
    ...
    /* ── se il 1° token è G/M, aggiorna lo stato modale ───────── */
    if (/^[GM]\d+/.test(token0)) {
      const c = token0.replace(/^G0([0-4])$/, 'G$1'); // G00→G0...

      if (/^G0[1234]|^G[01234]/.test(c)) {                   // G0-G4
        state.code =
          c.replace('G00','G0').replace('G01','G1')
           .replace('G02','G2').replace('G03','G3')
           .replace('G04','G4');
        parts.shift();                                       // rimuovi

      } else if (c === 'G94' || c === 'G95') {               // feed
        state.feedMode = c;
        parts.shift();

      } else if (c === 'G96' || c === 'G97') {               // Vc / RPM
        // state.speedMode = c  (non serve salvarlo)
        /* NON */ parts.shift();  // deve restare → cmd.code = G96/G97

      } else if (c === 'M03' || c === 'M3') {                // M3 = G97
        // state.speedMode = 'G97';
        parts.shift();

      } // G26 / G50 / G92 restano per la S-limite
    }

    /* ── costruisci il comando, ereditando il movimento corrente ─ */
    const cmd = {
      code: state.code,
...
/* 2) lunghezza arco incrementale I/K (per G2/G3) */
...
function computeLatheTime(cmds, userMax = Infinity) {
  ...
  for (const c of cmds) {
    ...
    if (c.code === 'M3'  && c.S != null) rpm = Math.min(c.S, rpmMax);
    if (c.code === 'G96' && c.S != null) Vc  = c.S;

    /* salta cambio utensile */
    if (c.L) continue;
    ...
    if (c.code === 'G1' || c.code === 'G0') {
      dist = Math.hypot((c.X ?? pos.X) - pos.X,
                        (c.Z ?? pos.Z) - pos.Z);
