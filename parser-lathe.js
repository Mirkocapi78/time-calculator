// parser-lathe.js – parser modale + calcolo tempo con limite giri utente

/* ---------- PARSER ---------- */
function parseISO(text) {
  const cmds = [];
  let state = { code: 'G0', feedMode: 'G95' };

  for (const raw of text.split(/\r?\n/)) {
    let line = raw.split(/;|\(/)[0].trim().replace(/^[NO]\d+\s*/i, '');
    if (!line) continue;

    const parts = line.toUpperCase().split(/\s+/);
    let token0 = parts[0];

    if (/^[GM]\d+/.test(token0)) {
      const c = token0.replace(/^G0([0-4])$/, 'G$1');

      if (/^G0[1234]|^G[01234]/.test(c)) {
        state.code = c.replace('G00','G0').replace('G01','G1')
                      .replace('G02','G2').replace('G03','G3')
                      .replace('G04','G4');
        parts.shift();
      } else if (c === 'G94' || c === 'G95') {
        state.feedMode = c;
        parts.shift();
      } else if (c === 'M03' || c === 'M3') {
        // trattiamo M3 come G97 costante, ma rimuoviamo il token
        parts.shift();
      } else if (c === 'G96' || c === 'G97') {
        // resta nel blocco (serve a computeLatheTime)
      }
      /* G26 / G50 / G92 restano per portare S-limite */
    }

    const cmd = {
      code: state.code,
      feedMode: state.feedMode,
      X:null, Z:null, I:null, K:null, F:null, S:null, P:null, L:null
    };

    for (const p of parts) {
      const k = p[0], v = parseFloat(p.slice(1));
      if (isNaN(v)) continue;
      if (k==='X') cmd.X=v; else if(k==='Z') cmd.Z=v;
      else if(k==='I') cmd.I=v; else if(k==='K') cmd.K=v;
      else if(k==='F') cmd.F=v; else if(k==='S') cmd.S=v;
      else if(k==='P') cmd.P=v; else if(k==='L') cmd.L=v;
    }
    cmds.push(cmd);
  }
  return cmds;
}

/* ---------- SUPPORT: lunghezza arco ---------- */
function arcLen(x0,z0,c){
  const xc = x0 + (c.I ?? 0), zc = z0 + (c.K ?? 0);
  const r  = Math.hypot(x0 - xc, z0 - zc);
  const x1 = c.X ?? x0,       z1 = c.Z ?? z0;
  let dθ   = Math.atan2(z1-zc,x1-xc) - Math.atan2(z0-zc,x0-xc);
  if (c.code==='G2' && dθ>0) dθ -= 2*Math.PI;
  if (c.code==='G3' && dθ<0) dθ += 2*Math.PI;
  return Math.abs(r * dθ);
}

/* ---------- CALCOLO TEMPO ---------- */
function computeLatheTime(cmds, userMax = Infinity) {
  const RAPID = 10000;
  let pos = { X:0, Z:0 };
  let feedRev = 0, rpm = 0, Vc = 0;
  let rpmMax  = Math.min(userMax, 4000);
  let tMin = 0;

  for (const c of cmds) {
    if (c.F != null) feedRev = c.F;
    if (['G26','G50','G92'].includes(c.code) && c.S != null)
      rpmMax = Math.min(userMax, c.S);

    if (c.code === 'G97' && c.S != null) rpm = Math.min(c.S, rpmMax);
    if (c.code === 'M3'  && c.S != null) rpm = Math.min(c.S, rpmMax);
    if (c.code === 'G96' && c.S != null) Vc  = c.S;

    if (c.L) continue;                              // skip cambio utensile

    if (c.code === 'G4') {                          // dwell
      const sec = c.X ?? c.F ?? c.P ?? 0;
      tMin += sec / 60;
      continue;
    }

    let dist = 0;
    if (c.code === 'G1' || c.code === 'G0')
      dist = Math.hypot((c.X ?? pos.X) - pos.X, (c.Z ?? pos.Z) - pos.Z);
    else if (c.code === 'G2' || c.code === 'G3')
      dist = arcLen(pos.X, pos.Z, c);

    if (c.code === 'G0')
      tMin += dist / RAPID;
    else if (['G1','G2','G3'].includes(c.code)) {
      if (Vc && pos.X > 0) {
        const rpmCalc = (1000 * Vc) / (Math.PI * pos.X);
        rpm = Math.min(rpmCalc, rpmMax);
      }
      const feedMMmin = (c.feedMode==='G95') ? feedRev*rpm : feedRev;
      if (feedMMmin > 0) tMin += dist / feedMMmin;
    }
    pos = { X: c.X ?? pos.X, Z: c.Z ?? pos.Z };
  }
  return tMin * 60;   // secondi
}

module.exports = { parseISO, computeLatheTime };
