...
function parseISO(text) {
  ...
  for (let line of text.split(/\r?\n/)) {
    ...
    if (iff) {
      raw.push({ type:'if', varName: iff[1].toUpperCase(), operator: iff[2], value: parseFloat(iff[3]), target: iff[4] });
      continue;
    }
 ...
 *    - espandi drill-cycle Siemens: MCALL CYCLExx(params)
 *    - ignora il nome S e F visti prima: li verranno letti in compute
 *    - riporta sequenze di G0/G1 per ogni ciclo
 */
  ...
  while (i<raw.length) {
    ...
    switch(r.type) {
      ...
      case 'if':
        ...
        break;
      case 'repeat':
        const start=labels[r.block], end=labels['ENDLABEL']||raw.length;
        for(let c=0;c<r.count;c++){
          for(let j=start;j<end;j++) if(raw[j].type==='command') cmds.push(raw[j].line);
        }
        i++; break;
      case 'command':
        const line=r.line;
        ...
        // se dentro ciclo e coordinata XY
        let m2=line.match(/^X([-\d.]+)\s+Y([-\d.]+)/i);
        if(cycle && m2) {
          const x=parseFloat(m2[1]), y=parseFloat(m2[2]);
          // espandi sequenza drill
...
function computeMillTime(cmdLines) {
  ...
  for(const line of cmdLines){
    ...
    for(let j=1;j<parts.length;j++){
      const p=parts[j]; const k=p[0].toUpperCase(); const v=parseFloat(p.slice(1));
      if(!isNaN(v)) args[k]=v;
    }
